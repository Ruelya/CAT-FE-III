...
/* The output is committed, so it has to satisfy `pnpm format:check` the
   moment it is written rather than after a manual pass. */
async function writeCss(path, source) {
  writeFileSync(path, await prettier.format(source, { parser: "css" }));
}
...
for (const theme of THEMES) {
  ...
  await writeCss(join(outDir, `${theme.id}.css`), out);
