              ...
              (blk) =>
                `<p class="preview__block">${blk
                  .map((n) => {
                    ...
                    /* A locked segment is confirmed and then frozen. The grid
                       stacks the two; a chip has one fill, so lock wins —
                       it is the fact that changes what a proofreader can do
                       with the segment. */
                    const st = s.locked ? "locked" : s.state;
                    const title = s.locked
                      ? `句段 #${n} · ${STATE_LABEL[s.state]} · 锁定`
                      : `句段 #${n} · ${STATE_LABEL[s.state]}`;
                    return `<button class="pvseg" data-state="${st}"${fb ? ' data-fallback="1"' : ""}${
                      n === S.active ? ' data-active="1"' : ""
                    } data-jump="${n}" title="${title}">${esc(fb ? s.src : s.tgt)}</button>`;
                  })
                  .join("")}</p>`,
            )
            .join("")}
