...
const S = {
  ...
  settingsTab: "info",
  cascade: false,
  expandedFolders: new Set(["f-source"]),
};
...
function vFileTree() {
  ...
  const renderItem = (item) => {
    if (item.type === "folder") {
      const isExpanded = S.expandedFolders.has(item.id) || query.length > 0;
      const matchingChildren = item.children.filter((child) =>
        query.length === 0 || child.name.toLowerCase().includes(query)
      );
      if (query.length > 0 && matchingChildren.length === 0 && !item.name.toLowerCase().includes(query)) {
        return "";
      }
      return `<div class="tree__folder" data-folder="${item.id}">
        <div class="tree__folder-head" data-act="toggle-folder" data-folder="${item.id}">
          <span class="tree__chevron">${ic(isExpanded ? "down" : "right", 11)}</span>
          <span class="tree__folder-icon">${ic("folder", 13)}</span>
          <span class="tree__folder-name">${esc(item.name)}</span>
          ${item.cloud ? `<span class="tree__cloud-icon" title="云端交付物">${ic("import", 11)}</span>` : ""}
        </div>
        ${
          isExpanded
            ? `<div class="tree__folder-body">${item.children
    ...
    const isActive = item.id === S.activeDoc;
    const isArmed = S.removeArmed === item.id;
    const pct = item.total ? Math.round(((item.confirmed || 0) / item.total) * 100) : null;
    const docCmd = `open-doc-${item.id.replace("doc-", "").replace("ref-", "").replace("del-", "")}`;
    const badgeCls = `file-badge file-badge--${item.badgeColor || "default"}`;

    return `<div class="tree__file-row"${isActive ? ' data-active="1"' : ""}>
      <button class="tree__file-btn" data-cmd="${docCmd}">
        <span class="${badgeCls}">${esc(item.badge || "DOC")}</span>
        <span class="tree__file-name">${esc(item.name)}</span>
        ${
          pct !== null
            ? `<span class="tree__file-pct num">${pct}%</span>`
            : item.total
            ? `<span class="tree__file-pct num">${item.total}条</span>`
            : ""
        }
      </button>
      ${
        !item.readonly
          ? isArmed
            ? `<span class="tree__file-confirm"><button class="btn btn--danger btn--xs" data-act="remove-yes">确认</button><button class="btn btn--ghost btn--xs" data-act="remove-no">取消</button></span>`
            : `<button class="tree__file-remove iconbtn iconbtn--xs" data-act="remove-arm" data-doc="${item.id}" title="移除文件">${ic("trash", 12)}</button>`
          : ""
      }
    </div>`;
  };

  return `<div class="tree">${FILE_TREE.map(renderItem).join("")}</div>`;
}
...
function vLeft() {
  ...
  return `<aside class="rail rail--left"${S.leftCollapsed ? ' data-collapsed="1"' : ""}>
    <section class="railsec railsec--project">
    ...
    <section class="railsec railsec--files">
      ...
      <label class="railsearch">${ic("search", 13)}
        <input id="filesearch" type="search" placeholder="搜索文件与资源" aria-label="搜索文件" value="${esc(S.fileQuery)}">
      </label>
      ${vFileTree()}
    </section>
...
function view() {
  ...
  return `
  <div class="proto">
    <span class="proto__brand">Translunar CAT · <b>${THEME_NAME}</b></span>
...
function onClick(e) {
  ...
  if (act) {
    switch (act) {
      ...
      case "toggle-folder":
        {
          const fid = t.dataset.folder;
          if (S.expandedFolders.has(fid)) S.expandedFolders.delete(fid);
          else S.expandedFolders.add(fid);
          return render();
        }
      case "toggle-left":
        S.leftCollapsed = !S.leftCollapsed;
