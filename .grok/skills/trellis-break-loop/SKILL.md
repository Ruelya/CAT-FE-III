...
description: "Deep bug analysis to break the fix-forget-repeat cycle. Primary loader: trellis-fix subagent when the same finding class keeps failing. Also usable after hard debug. Captures prevention into review notes and/or specs."
---

# Break the Loop - Deep Bug Analysis

**Primary owner in Grok Autoloop:** `trellis-fix` loads this skill when the same issue class fails repeated fixes. Write analysis to `{TASK_DIR}/review/break-loop-<topic>.md`. Large spec harvests may be finished by `trellis-closeout` + `trellis-update-spec`.

When debug is complete (or thrashing), use this for deep analysis to break the "fix bug -> forget -> repeat" cycle.
...
**IMPORTANT**: Persist the analysis; do not leave it only in chat.

1. **Always** write `{TASK_DIR}/review/break-loop-<topic>.md` (or equivalent under the active task) with the full analysis template above.
2. **If you are `trellis-fix`:** apply in-scope code prevention; only touch `.trellis/spec/` for a small local guardrail. Do **not** git commit (Orchestrator commits).
3. **If you are `trellis-closeout` or running update-spec:** fold durable lessons into `.trellis/spec/` / guides via `trellis-update-spec`.
4. Do **not** require template sync under `src/templates/...` unless this repo still uses that path for the area you edited.

> **The analysis is worthless if it stays in chat. The value is in task review files and updated specs.**
