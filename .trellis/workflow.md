...
- `implement` — Claude implementation worker (see root `AGENTS.md`)
- `check` — default check worker (stock card / Claude unless overridden)
- `check-cc` — Claude check worker
- `check-cx` — Codex check worker

**CAT policy:** implementation and quality checks use Claude channel workers by
default (session policy in root `AGENTS.md` + `.trellis/agents/implement.md`).
Implementation workers edit the shared worktree directly with focused changes.
