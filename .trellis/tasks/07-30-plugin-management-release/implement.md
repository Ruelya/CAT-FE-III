...
- [ ] Add closed distribution metadata and source/materialized package types.
- [ ] Implement deterministic `.tlplugin` extraction with all path/link/
      collision/compression/count/depth/byte guards before writes.
- [ ] Route directory/archive inspect and stage through one validate/hash path;
      prove equal canonical hashes and cleanup on every error.
- [ ] Add public SDK types/validators/build helpers without breaking manifest v1
...
- [ ] Add migration 24 for active-version and immutable-version distribution
      provenance/metadata with deterministic legacy backfill and restart tests.
- [ ] Extend inspection, summary, and version projections; add bounded bundled
      catalog/apply contracts and method catalog entries.
- [ ] Regenerate schema/TypeScript contracts and run drift checks.

## 3. Engine Bundle and Lifecycle Integration

Owner scope: Engine CLI/service/plugin lifecycle and focused smoke fixtures.

- [ ] Accept an optional trusted bundled-plugin root at Engine startup and pass
      the packaged resources path from Electron main.
- [ ] Load/validate the closed generated index and expose bounded list/apply;
      infer source kind from canonical materialization context only.
- [ ] Refactor inspect/install/upgrade around the shared materializer; re-hash
      managed candidates before persistence.
- [ ] Verify package hash on restart and rollback; degrade/detach only the exact
      corrupt generation with bounded diagnostics.
- [ ] Reuse existing permission diff, candidate preflight, registry attach,
...
- [ ] Add deterministic pack/check/index scripts with stable entry order,
      timestamps, modes, hashes, and allowlist validation.
- [ ] Add publisher/license metadata and license text to every released example;
      exclude credentials and test-only fixtures from the core catalog.
- [ ] Package the generated offline catalog as desktop resources and add drift,
...
- [ ] Update the trusted picker for directories and `.tlplugin` files.
- [ ] Add inspection confirmation and bundled catalog states without exposing
      bundled resource paths to React.
- [ ] Add local upgrade, version history, rollback, source/license/hash/
      compatibility/crash/diagnostic projections, and complete contribution
      inventory.
- [ ] Preserve permission-review authority, exact revisions, panel teardown,
      typed errors, focus traps, keyboard operation, and responsive layout.
- [ ] Add English/Simplified Chinese strings and focused component/state tests.

## 6. End-to-End Qualification and Evidence

- [ ] Extend real Engine smoke through archive inspect/install, permission review,
      enable/use/restart, upgrade, rollback, revoke, disable, uninstall, bundled
      apply/restore, failure compensation, and post-failure health.
- [ ] Extend Electron E2E through local and bundled workflows, stale revision,
      diagnostics, version history, rollback, and uninstall with console checks.
- [ ] Capture/inspect 1250x744, 1680x942, and 1920x1080 evidence for installed,
      bundled, inspection, permission, and version-history states.
- [ ] Create `acceptance-evidence.md` mapping R1-R6 and AC-01-AC-12 to exact
...
- [ ] Independent check explicitly reviews archive extraction attacks, bundled
      provenance spoofing, hash/revision races, exact-generation compensation,
      legacy migration, renderer trust boundaries, and public-vs-fixture release
      inputs.
- [ ] Generated contracts and built example artifacts show no drift.
- [ ] No credential, absolute private path, source/target text, raw plugin payload,
      or stack trace appears in persistence, catalog, errors, diagnostics, logs,
      screenshots, or evidence.
- [ ] No marketplace, remote signing/index, app updater, or OS-sandbox claim has
      entered scope.
