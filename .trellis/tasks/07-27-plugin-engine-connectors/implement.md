...
- [x] Confirm archived multi-tier, permission, Tier 1, and Tier 2 task evidence
      and load the curated backend/frontend specs plus `research/baseline.md`.
- [x] Re-read complete current plugin lifecycle, capability authorizer,
      `AiManager`, provider profile/run/batch storage, protocol generation, SDK,
      and Desktop AI/Plugins definitions before editing.
- [x] Review this final planning set and run `task.py start` only after the main
      session receives implementation approval.

## Ordered Implementation

1. [x] Define strict connector descriptor/definition/request/event/result/error
...
2. [x] Refactor `ai-core` around a provider-neutral `EngineConnector` adapter
       and owner-aware registry; wrap every current built-in provider first and
       prove unchanged catalog/request/error behavior.
3. [x] Add an append-only storage migration and protocol source union for exact
...
4. [x] Implement Tier 1 declarative connector definitions and bounded host HTTP
...
5. [x] Implement Tier 2 sandbox and Tier 3 process connector adapters using the
       ...
       diagnostic mapping.
6. [x] Integrate connector prepare/attach/detach with reload, enable, disable,
...
7. [x] Route provider catalog/profile CRUD/test, interactive/action execution,
...
8. [x] Keep credentials Engine/keyring-owned; add operation/origin authorization,
...
9. [x] Extend public SDK builders/validators/server helpers and ship deterministic
...
10. [x] Update Desktop AI catalog/profile forms and Plugins inventory details
...
11. [x] Add layered unit/integration/restart/migration/race/security tests, real
...
12. [x] Run the complete gate, resolve check findings, update Trellis specs,
...
- [x] Built-in profile/catalog/request/error/usage behavior is regression-tested
      before plugin adapters are advertised.
- [x] No secret, prompt, source/target text, package path, stack, or runtime
      handle reaches SQLite, logs, audit, diagnostics, generated errors, or UI.
- [x] Every public collection/payload/event/timeout has a named limit and a
      boundary test; every operation is a finite enum with a closed codec.
- [x] Every registry/lifecycle mutation has exact owner/version cleanup and a
      tested compensation path; cancellation wins races without partial apply.
- [x] Tier 3 trust documentation does not claim OS isolation, and Tier 2 does
      not expose generic network or Engine invocation.
- [x] Production Electron screenshots are inspected at all required viewports;
...
- [x] AC-01 through AC-12 map to exact tests, commands, and screenshots in
      task-owned evidence.
- [x] The final full gate runs after the last production-code change.
- [x] Spec updates, implementation, evidence, commit, and archive contain no
      unrelated dirty-worktree files.

## Completion Record

- Final full workspace gates passed after the last production-code change:
  contracts drift, Prettier/Rust format, ESLint/strict Clippy, workspace
  typecheck, `pnpm test`, Rust workspace tests, and docs checks.
- Focused real connector stdio smoke passed. Production Electron connector E2E
  passed install through uninstall, including exact profile rebind on upgrade
  and rollback, revoke/re-grant, explicit disable, zero console/page errors,
  and 1250x744, 1680x942, and 1920x1080 screenshots.
- Full Engine/Desktop E2E limits are recorded honestly in
  `evidence/qualification.md`; neither blocked/incomplete full suite is claimed
  as passed.
