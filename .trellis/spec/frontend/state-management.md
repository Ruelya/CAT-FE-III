...
> ownership rule itself still applies: the Rust engine (now persisting a
> rusqlite `engine.sqlite` database in `crates/tl-engine/src/store.rs`) is
> authoritative and React state is a presentation cache.
