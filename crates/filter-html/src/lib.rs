...
fn build_units(
...
) -> Result<(Vec<TextUnit>, Vec<translunar_domain::DegradationFinding>), FilterError> {
    ...
    if parsed.kind == HtmlKind::Html && stack.iter().any(|item| !is_void(&item.1)) {
        degradation.push(translunar_domain::DegradationFinding {
            code: "html-unclosed-element".to_string(),
            severity: translunar_domain::DegradationSeverity::Warning,
...
mod tests {
    ...
    fn units(path: &Path) -> Vec<translunar_filter_core::ImportedUnit> {
    ...
    fn valid_xhtml_namespace_and_attribute_configuration_round_trip() {
        ...
        let stream = HtmlFilter
            ...
            .expect("import XHTML");
        let document =
            translunar_filter_core::collect_imported_document(stream).expect("collect XHTML units");
        assert!(
            document
                .units
                .iter()
