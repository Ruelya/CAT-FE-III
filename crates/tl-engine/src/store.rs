...
/// `memory_id`, `source_hash`, and `source_text` are immutable per entry id;
/// the unique `(memory_id, source_hash)` index backs the engine's exact-match
/// invariant of at most one entry per normalized source in a memory.
