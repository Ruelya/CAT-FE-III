...
function validateInvocationCommon(record, protocol, errors) {
...
function validateContributionCancelRequest(value) {
  const errors = [];
...
function writeRpcNotification(method, params) {
  output.write(`${JSON.stringify({ jsonrpc: "2.0", method, params })}
`);
}
function safeProcessFailure(error, invocationId) {
  if (
...
function writeRpcFailure(id, failure) {
  output.write(
    `${JSON.stringify({ jsonrpc: "2.0", id, error: { code: -32020, message: "plugin contribution invocation failed", data: failure } })}
`,
  );
}
function startProcessQaPipelinePlugin(options) {
  const qaDescriptors = /* @__PURE__ */ new Map();
  const pipelineDescriptors = /* @__PURE__ */ new Map();
  for (const contribution of options.manifest.contributions) {
    if (
      ...
      qaDescriptors.set(contribution.id, contribution);
    }
    if (
      ...
      pipelineDescriptors.set(contribution.id, contribution);
    }
  }
...
async function handleQaPipelineProcessLine(
  ...
  const id = typeof request.id === "number" ? request.id : void 0;
  ...
  try {
    ...
    if (request.method === "plugin.shutdown") {
      ...
      if (id !== void 0) writeRpcResult(id, {});
    ...
    if (request.method === "qa.evaluateSegment") {
      ...
      try {
        ...
        if (id !== void 0) writeRpcResult(id, result);
      } finally {
    ...
    if (request.method === "pipeline.checkpointMigrate") {
      ...
      try {
        const result = await withInvocationSignal(
          raw.invocationId,
          /* @__PURE__ */ new Map(),
          (context) => handler.migrateCheckpoint?.(raw, context),
          controller,
        );
        throwErrors(
        ...
        const result = await withInvocationSignal(
          ...
          (context) => {
            ...
            if (raw.operation === "execute")
              return handler.execute(raw, pipelineContext);
            if (!handler.resume)
              throw new PipelineStepHandlerError({
                protocolVersion: 1,
                invocationId: raw.invocationId,
            ...
            return handler.resume(raw, pipelineContext);
          },
          controller,
        );
        throwErrors(
...
// ../../examples/plugins/qa-pipeline-process/src/index.ts
...
function recordsFromInvocation(invocation) {
  const payload = configRecord(invocation.input.value);
  const records = payload.records;
  return Array.isArray(records)
    ? records.filter((value) => typeof value === "string")
    : [];
}
function checkpointCursor(invocation) {
...
function evaluateBrand(invocation) {
...
async function normalizeBatch(invocation, context) {
...
function migrateCheckpoint(invocation) {
